...
TARGET_CYCLES = 12   # 12 total, so cycle 1 (may be contaminated by the
                      # initial transient before the first real crossing)
                      # and the last cycle (may be cut short by the stop
                      # condition) can both be dropped, leaving 10 clean
                      # cycles for the final mean/analysis
...
def main():
            ...
            if trial_state == "instructions":
                ...
                if cop_mm is not None:
                    ...
                    if current_sign is not None:
                        # Only start tracking peaks once we've had at
                        # least one crossing - the initial transient
                        # (small wiggle near center before the person's
                        # first real excursion past the threshold) is
                        # NOT part of any real cycle and must not
                        # contaminate cycle 1's peaks. Without this
                        # guard, cycle 1 can end up with a tiny/wrong
                        # peak on one side, because peak tracking would
                        # start from frame 1 instead of from the first
                        # genuine crossing.
                        if zero_crossings > 0:
    ...
    if cycle_peaks:
        ...
        for cyc, peak_pos, peak_neg in cycle_peaks:
            note = ""
            if cyc == cycle_peaks[0][0]:
                note = "  <- excluded (may include initial transient before first crossing)"
            elif cyc == cycle_peaks[-1][0]:
                note = "  <- excluded (may be cut short by trial stop condition)"
            print(f"  {cyc:6d} {peak_pos:12.1f} {peak_neg:12.1f}{note}")

        # Drop the first and last cycle before computing the mean: cycle
        # 1 can be contaminated by the initial transient (the small
        # wiggle near center before the person's first real excursion),
        # and the last cycle can be cut short since the trial stops as
        # soon as TARGET_CYCLES is reached, not necessarily at a natural
        # cycle boundary. This is why the task runs to TARGET_CYCLES=12
        # instead of 10 - so 10 clean cycles remain after dropping both.
        if len(cycle_peaks) > 2:
            clean_peaks = cycle_peaks[1:-1]
            excluded_cycle_numbers = {cycle_peaks[0][0], cycle_peaks[-1][0]}
        else:
            print("\n  Warning: fewer than 3 cycles completed - cannot drop first/last, "
                  "using all available cycles for the mean instead.")
            clean_peaks = cycle_peaks
            excluded_cycle_numbers = set()

        mean_peak_pos = sum(p[1] for p in clean_peaks) / len(clean_peaks)
        mean_peak_neg = sum(p[2] for p in clean_peaks) / len(clean_peaks)
        print(f"\n  Mean peak right (excluding first/last cycle, {len(clean_peaks)} cycles used): {mean_peak_pos:.1f}mm")
        print(f"  Mean peak left  (excluding first/last cycle, {len(clean_peaks)} cycles used): {mean_peak_neg:.1f}mm")
